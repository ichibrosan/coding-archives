#!/usr/bin/python
# render.py 2008/02/18 COPR douglas_goodall@mac.com
# 2008/02/28 15:32 dwg - add oval spheroid inside quadrant 1 box
import ConfigParser
import os
import random
import string
...
def plot_cube(x,y,z):
	b = int(cfg.get("canvas","border")) + 10
	qw = ( CW - b - b - b - b ) / 2 ; qh = ( CH  - b - b - b - b ) / 2
	blipx = ( qw - b - b ) / 13 ; blipy = ( qh - b - b ) / 11 ; r = 35 ; d = 30
	canvas.create_line(r+b+(blipx*0),d+b+(blipy*11),r+b+(blipx*5),d+b+(blipy*8),fill="gray")
	canvas.create_line(r+b+(blipx*5),d+b+(blipy*8),r+b+(blipx*5),d+b+(blipy*0),fill="gray")
	canvas.create_line(r+b+(blipx*5),d+b+(blipy*8),r+b+(blipx*13),d+b+(blipy*8),fill="gray")

	# back
	canvas.create_polygon(
		r+b+(blipx*5),d+b+(blipy*0),r+b+(blipx*13),d+b+(blipy*0),
		r+b+(blipx*13),d+b+(blipy*8),r+b+(blipx*5),d+b+(blipy*8),
		r+b+(blipx*5),d+b+(blipy*0),fill=random_color())

	# left
	canvas.create_polygon(
		r+b+(blipx*0),d+b+(blipy*11),r+b+(blipx*5),d+b+(blipy*8),
		r+b+(blipx*5),d+b+(blipy*0),r+b+(blipx*0),d+b+(blipy*3),
		r+b+(blipx*0),d+b+(blipy*11),fill=random_color())

	# bottom
	canvas.create_polygon(
		r+b+(blipx*0),d+b+(blipy*11),r+b+(blipx*8),d+b+(blipy*11),
		r+b+(blipx*13),d+b+(blipy*8),r+b+(blipx*5),d+b+(blipy*8),
		r+b+(blipx*0),d+b+(blipy*11),fill=random_color())
	
	# front lower left to front lower right
	canvas.create_line(	r+b+(blipx*0),d+b+(blipy*11),
									r+b+(blipx*8),d+b+(blipy*11),
									fill=random_color(),width=3)
	
	# front lower left to front upper left
	canvas.create_line(	r+b+(blipx*0),d+b+(blipy*11),
									r+b+(blipx*0),d+b+(blipy*3),
									fill=random_color(),width=3)
		
	# front upper left to rear upper left
	canvas.create_line(	r+b+(blipx*0),d+b+(blipy*3),
									r+b+(blipx*5),d+b+(blipy*0),
									fill=random_color(),width=3)
	
	# rear upper left to rear upper right
	canvas.create_line(	r+b+(blipx*5),d+b+(blipy*0),
									r+b+(blipx*13),d+b+(blipy*0),
									fill=random_color(),width=3)
	
	# front lower right to rear lower right
	canvas.create_line(	r+b+(blipx*8),d+b+(blipy*11),
									r+b+(blipx*13),d+b+(blipy*8),
									fill=random_color(),width=3)
	
	# rear lower right to rear upper right
	canvas.create_line(	r+b+(blipx*13),d+b+(blipy*8),
									r+b+(blipx*13),d+b+(blipy*0),
									fill=random_color(),width=3)

	# render object into box before drawing the last three lines:
	#		front upper left to front upper right
	# 		front upper right to rear upper right
	#		front upper right to front lower right

	# draw internal object ( larger globe )
	canvas.create_oval(
								r+b+(blipx*3), d+b+(blipy*2),
								r+b+(blipx*11), d+b+(blipy*10),
								fill=random_color())
		
	# draw internal object ( smaller globe )
	canvas.create_oval(
								r+b+(blipx*1), d+b+(blipy*7),
								r+b+(blipx*5), d+b+(blipy*10),
								fill=random_color())

	# front upper left to front upper right
	canvas.create_line(	r+b+(blipx*0),d+b+(blipy*3),
									r+b+(blipx*8),d+b+(blipy*3),
									fill="black",width=3)

	# front upper right to rear upper right
	canvas.create_line(	r+b+(blipx*8),d+b+(blipy*3),
									r+b+(blipx*13),d+b+(blipy*0),
									fill="black",width=3)
...
def view_portal():	
	...
	canvas.create_rectangle(q1x1, q1y1, q1x2, q1y2,fill="white")
	canvas.create_rectangle(q2x1, q2y1,q2x2,q2y2,fill="gray")
	canvas.create_rectangle(q3x1, q3y1,q3x2,q3y2,fill="gray")
	canvas.create_rectangle(q4x1, q4y1,q4x2,q4y2,fill="white")
