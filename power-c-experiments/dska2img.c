// dska2img.c 2009/11/29 dwg - 
// Copyright (C) 2009 Douglas W. Goodall. All Rights Reserved.

// The purpose of this program is to read a 3-1/2" 1.44mb
...
int main(int argc,char **argv)
{
	...
	printf("--------------------------------------------------\n");
	printf("DskA2Img.exe(c)           2009/12/01  Version 0.0a\n");
	...
	for(track=0;track<TRKS;track++) {
		for(head=0;head<HDS;head++) {
			...
			rc = biosdisk(INT13_READ,0,head,track,1,SPT/2,buffer1);
			if(0 != rc) {
				printf("1st read error is 0x%02x\n",rc);
				exit(EXIT_FAILURE);
			}
			fwrite(buffer1,512,SPT/2,fp);
			
			rc = biosdisk(INT13_READ,0,head,track,(SPT/2)+1,SPT/2,buffer2);
			if(0 != rc) {
				printf("2nd read error is 0x%02x\n",rc);
				exit(EXIT_FAILURE);
			}
			fwrite(buffer2,512,SPT/2,fp);
